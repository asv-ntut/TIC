#!/bin/bash
# ==============================================================================
# 衛星影像 壓縮/解壓縮 整合測試腳本 (PC Loopback Test) V2
...
# 1. 輸入影像路徑 (原始影像)
# 修改：根據您的 ls 結果，資料夾名稱應為 4096_3072
...
# --- 環境檢查 ---
# 自動偵測是否已在 Conda 環境中 (您目前在 tic10，這段邏輯會自動通過)
...
for input_path in "${PATH_ARRAY[@]}"; do
    ...
    if [ -d "$compressed_folder" ]; then
        ...
        # 執行 Python 解壓縮
        # 注意：--original 指向輸入的原始檔，確保 PSNR 計算正確
