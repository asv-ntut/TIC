...
# import wandb # 如果您沒有要用 wandb，可以註解掉
...
def init(args):
    base_dir = f'./distilled/{args.model}_q{args.quality_level}_from_{args.teacher_model}_q{args.teacher_quality}/'
...
def train_one_epoch(student_model, teacher_model, criterion, train_dataloader, optimizer, aux_optimizer, epoch,
    ...
    for i, d in enumerate(train_dataloader):
        ...
        if i % 100 == 0:
            logging.info(
                f"Train epoch {epoch} | [{i * len(d)}/{len(train_dataloader.dataset)}] | Total Loss: {total_loss.item():.4f} | Task Loss: {task_loss.item():.4f} | Distill Loss: {distill_loss.item():.6f}")
...
def parse_args(argv):
    ...
    parser.add_argument("-m", "--model", default="tic", choices=image_models.keys(), help="Student model architecture.")
    ...
    parser.add_argument("-q", "--quality-level", type=int, default=3, help="Student model quality level.")
...
def main(argv):
    ...
    logging.info(f"Creating student model: {args.model} (q={args.quality_level})")
    student_net = image_models[args.model](quality=int(args.quality_level)).to(device)
    ...
    if os.path.exists(best_checkpoint_path):
        ...
        # 處理 DataParallel 的 state_dict key
