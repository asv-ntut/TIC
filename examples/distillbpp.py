...
class SimpleConvStudentModel(nn.Module):
    ...
    def forward(self, x):
        y = self.g_a(x)
        y_adapted = self.adapter(y)  # 確定adapter 位置
        z = self.h_a(y)
...
class RateDistortionLoss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        out["bpp_loss"] = sum((torch.log(likelihoods).sum() / (-math.log(2) * num_pixels)) for likelihoods in
                              output["likelihoods"].values())  # 把likelihood拿出來
        out["mse_loss"] = self.mse(output["x_hat"], target)  # 把x_hat拿出來
...
def train_one_epoch(student_model, teacher_model, criterion, train_dataloader, optimizer, aux_optimizer, epoch,
    ...
    for i, d in enumerate(train_dataloader):
        ...
        student_out = student_model(d)  # 想知道裡面有啥 看673行
        task_loss_dict = criterion(student_out, d)  # criterion根據student out打分數
...
def main(argv):
    ...
    hook_handle = None;
    target_layer_name = 'g_a6'  # 2對齊teacher model ga6
